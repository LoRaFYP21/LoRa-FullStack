...
enum State : uint8_t { ST_IDLE = 0, ST_ARMED = 1, ST_RUNNING = 2, ST_DONE = 3 };
...
static void setRadio(uint8_t sf, long bw_hz) {
  ...
  if (CRC_ON) LoRa.enableCrc(); else LoRa.disableCrc();
...
static bool parseAck(const String &pkt, uint32_t &seq) {
  if (!pkt.startsWith("ACK,")) return false;
...
static void handleRunning() {
  if ((int32_t)(millis() - t0_ms) < 0) return;
...
static void handleSerialLine(String line) {
  ...
  if (u.startsWith("ROLE ")) {
    if (u.endsWith("TX")) { myRole = ROLE_TX; saveRole(myRole); }
    else { myRole = ROLE_RX; saveRole(myRole); }
...
void loop() {
  ...
  if (st == ST_RUNNING) handleRunning();
}
