...
bool sendMessageReliable(const String &lineIn)
{
    ...
    else
    {
        ...
        for (size_t i = 0; i < total; i++)
        {
            ...
            size_t hdrLen = 5 + myId.length() + 1 + dstAny.length() + 1 +
                            digitsU(seq) + 1 + digitsU(i) + 1 + digitsU(total) + 1;
        ...
        // Previously we waited for final ACK here.
        // Now we rely on fragment-level ACKF only (PC does full reassembly).
