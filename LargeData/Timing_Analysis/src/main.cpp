...
static void printFSInfo(){
  if(!SPIFFS.begin(false)){
    // Do not format automatically here, only report
    Serial.println("(FSINFO) SPIFFS not mounted");
    return;
  }
  size_t total = SPIFFS.totalBytes();
  size_t used  = SPIFFS.usedBytes();
  Serial.printf("SPIFFS: used %u / %u bytes (%.1f%%)\n", (unsigned)used, (unsigned)total, total? (100.0*used/total):0.0);
}

static bool remountSPIFFS(bool allowFormat){
  if(SPIFFS.begin(false)) return true; // already OK or mounted now (no format)
  if(!allowFormat){
    Serial.println("REMOUNT: SPIFFS.begin(false) failed. Use FORMAT_SPIFFS to erase+create.");
    return false;
  }
  Serial.println("Formatting SPIFFS (this will erase all files)...");
  if(!SPIFFS.format()){
    Serial.println("FORMAT failed");
    return false;
  }
  if(!SPIFFS.begin(false)){
    Serial.println("SPIFFS.begin after format still failed");
    return false;
  }
  Serial.println("FORMAT & mount OK");
  printFSInfo();
  return true;
}

// Initialize CSV logging files (requires SPIFFS already mounted)
bool initCSVLogging() {
  if(!SPIFFS.begin(false)){
    Serial.println("CSV logging disabled: SPIFFS not mounted (try REMOUNT or FORMAT_SPIFFS)");
    return false;
  }
  ...
  if (!txCsvFile || !rxCsvFile) {
    Serial.println("Failed to create CSV files (check free space)");
    csvLoggingEnabled = false;
    return false;
  }

  // Write headers
  const char* header = "time_ms,packet_type,sequence_no,fragment_idx,total_fragments,packet_size_bytes";
  txCsvFile.println(header);
  rxCsvFile.println(header);
  txCsvFile.flush();
  rxCsvFile.flush();

  Serial.println("CSV logging files created:");
...
void showHelp() {
  ...
  Serial.println("FSINFO        - Show SPIFFS usage");
  Serial.println("REMOUNT       - Retry SPIFFS.begin (no format)");
  Serial.println("FORMAT_SPIFFS - Erase + mount filesystem (DATA LOSS)");
...
void processSerialCommand(const String& command) {
  ...
  if (cmd == "HELP" || cmd == "?") {
    ...
    if(SPIFFS.begin(false)) Serial.println("REMOUNT: already mounted / success");
    else Serial.println("REMOUNT: failed (use FORMAT_SPIFFS)");
  } else if (cmd == "FORMAT_SPIFFS") {
    remountSPIFFS(true);
    Serial.println("Re-init CSV logging after format...");
    initCSVLogging();
...
void setup(){
  ...
  // Attempt a non-destructive mount (no format). User can invoke FORMAT_SPIFFS if needed.
  if(SPIFFS.begin(false)){
    Serial.println("SPIFFS mount OK");
    initCSVLogging();
  } else {
    Serial.println("SPIFFS mount FAILED (use REMOUNT or FORMAT_SPIFFS)");
  }
  ...
  Serial.println("   FSINFO          - Show filesystem usage");
  Serial.println("   REMOUNT         - Retry mounting SPIFFS");
  Serial.println("   FORMAT_SPIFFS   - Erase + mount (DATA LOSS)");
  ...
  Serial.println("   <text>          - Send LoRa message");
  Serial.println("");
}
